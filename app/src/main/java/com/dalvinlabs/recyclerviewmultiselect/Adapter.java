...
public class Adapter extends RecyclerView.Adapter<Adapter.MyViewHolder> {
  ...
  static String[] selected_branches=new String[100];
  static int i=0;
  private  final  List<Branches> products;
  private static SelectionTracker<Long> selectionTracker;

  Adapter(List<Branches> products) {
  ...
  public class MyViewHolder extends RecyclerView.ViewHolder {
    ...
    void bind(ViewModel viewModel, int position) {
      ...
      i=+1;
      selected_branches[i]=viewModel.name;

      binding.executePendingBindings();
      if (selectionTracker != null) {
        if (Adapter.this.selectionTracker.isSelected(details.getSelectionKey())) {
         // binding.getViewModel().textColor.set(R.color.green);
          binding.getRoot().setActivated(true);
          binding.foreground.setBackgroundColor(R.color.colorAccent);

        } else {
         // binding.getViewModel().textColor.set(R.color.white);
          binding.getRoot().setActivated(false);
            binding.foreground.setBackgroundColor(R.color.white);
        }
      }
    }
